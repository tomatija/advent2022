...
void executeCrateMove(int startStack, int destinationStack, vector<Stack>& stacks){
    string crateToMove = stacks[startStack].takeCrateFromStack();
    ...
    while (getline (InputFile, myText)) {
        ...
        if(readyToRead){
            ...
            if(splitElements.size() > 0){
                for(int i = 0; i < stoi(splitElements[1]); i++){
                    executeCrateMove(stoi(splitElements[3]) - 1, stoi(splitElements[5]) - 1, stacks);
                }
            }
        }
    }
